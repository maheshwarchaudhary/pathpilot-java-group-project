    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8" id="resourceGrid">
            ...
            <c:forEach var="pathInfo" items="${careerPaths}">
                ...
                <%-- Assign colors and icons based on path title --%>
                ...
                <c:choose>
                    ...
                    <c:when test="${fn:containsIgnoreCase(path.title, 'DevOps') || fn:containsIgnoreCase(path.title, 'Cloud') || fn:containsIgnoreCase(path.title, 'AWS')}">
                        <c:set var="bgColor" value="blue-600" />
                    ...
                    <c:when test="${fn:containsIgnoreCase(path.title, 'Data') || fn:containsIgnoreCase(path.title, 'AI') || fn:containsIgnoreCase(path.title, 'ML')}">
                        <c:set var="bgColor" value="pink-600" />
                        <c:set var="iconClass" value="fa-brain" />
                        <c:set var="category" value="development" />
                    </c:when>
                </c:choose>

                <div class="resource-card domain-card bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-xl hover:border-primary/20" data-category="${category}">
                    <div class="h-52 bg-${bgColor} flex items-center justify-center p-8 relative overflow-hidden">
                        <div class="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>
                        <span class="absolute top-4 left-4 bg-white/20 backdrop-blur-md text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest">${path.category != null ? path.category : 'TECH'}</span>
                        <i class="fas ${iconClass} text-white text-6xl opacity-40 domain-icon transition-transform duration-500"></i>
                        <c:if test="${isEnrolled}">
                            <div class="absolute top-4 right-4 bg-green-100 text-green-700 px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1">
                    ...
                    <div class="p-8">
                        ...
                        <div class="flex items-center justify-between pt-5 border-t border-gray-50">
                            <div class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full ${isEnrolled ? 'bg-green-500' : 'bg-blue-500'}"></span>
                                <span class="text-[10px] text-gray-400 font-black uppercase">${path.totalPhases} Phases ${isEnrolled ? 'Active' : 'Available'}</span>
                            </div>
                            <a href="<%=request.getContextPath()%>/user/view-path?id=${path.pathId}" class="text-primary font-bold text-sm hover:underline">
