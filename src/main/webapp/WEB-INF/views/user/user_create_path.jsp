    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-100 px-12 py-5 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-800 text-gray-900 tracking-tight">Launch New Roadmap</h1>
            </div>
        ...
        <main class="flex-1 overflow-y-auto pb-32">
            <form id="roadmapForm" action="<%=request.getContextPath()%>/user/create-path" method="POST" enctype="multipart/form-data" class="max-w-5xl mx-auto px-12 py-12">
                ...
                <section class="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm mb-12">
                    ...
                    <div class="grid md:grid-cols-2 gap-8">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="roadmapTitle" class="input-field validate-required" placeholder="e.g. Full Stack Web Engineering">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="roadmapDesc" rows="3" class="input-field resize-none validate-required" placeholder="Provide a brief overview..."></textarea>
...
<script>
    ...
    function addPhase() {
        ...
        // Get all question blocks from the cloned template
        const questions = template.querySelectorAll('.question-block');
        console.log(`📌 Phase ${phaseCount}: Cloned template contains ${questions.length} questions`);
        
        questions.forEach((q, qIndex) => {
            const qNum = qIndex + 1;
            console.log(`  Updating Question ${qNum}...`);
            ...
            console.log(`    ✓ Updated ${radios.length} radio buttons`);
            
            // Update question input name - get the first text input in this question block
            const textInputs = q.querySelectorAll('input[type="text"]');
            if (textInputs.length > 0) {
                // First text input is the question text
                textInputs[0].name = "phaseQuiz_P" + phaseCount + "_Q" + qNum + "[]";
                console.log(`    ✓ Updated question input`);
            }
            
            // Update option input names - remaining text inputs are options A, B, C, D
            ...
            console.log(`    ✓ Updated ${Math.min(4, textInputs.length - 1)} option inputs`);
        });

        container.appendChild(template);
        console.log(`✅ Phase ${phaseCount} added successfully with ${questions.length} questions\n`);
        
        // Setup file upload listeners for newly added phase
        const fileInputs = container.querySelectorAll('.file-input');
        fileInputs.forEach(input => {
            setupFileUploadListener(input);
        });
    ...
    function validateForm() {
        console.log("🔍 [VALIDATION START] Starting form validation...");
        let isValid = true;
        ...
        console.log("📋 [CHECK 1] Validating required fields...");
        form.querySelectorAll('.validate-required').forEach(input => {
            const value = input.value.trim();
            const fieldName = input.name || input.getAttribute('placeholder') || 'Unknown Field';
            
            if (!value) {
                console.warn(`❌ [EMPTY FIELD] "${fieldName}" is empty`);
                showError(input);
                validationErrors.push(`Required field "${fieldName}" is empty`);
                isValid = false;
            } else {
        ...
        console.log("🔗 [CHECK 2] Validating URLs...");
        const urlRegex = /^(https?:\/\/)?(www\.)?([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}([\/\w\-._~:/?#[\]@!$&'()*+,;=]*)?$/;
        form.querySelectorAll('.validate-url').forEach(input => {
            const value = input.value.trim();
            const fieldName = input.getAttribute('placeholder') || 'URL Field';
            
            // Only validate if field has a value (optional field)
            if (value.length > 0) {
                if (!urlRegex.test(value)) {
                    console.warn(`❌ [INVALID URL] "${fieldName}": "${value}"`);
                    showError(input);
                    validationErrors.push(`Invalid URL in "${fieldName}": ${value}`);
                    isValid = false;
                } else {
                    console.log(`✅ [VALID URL] "${fieldName}": "${value}"`);
                }
            } else {
                console.log(`⏭️ [OPTIONAL URL] "${fieldName}" is empty (optional field - skipping validation)`);
            }
        });

        // ===== CHECK 3: Radio Button Selection =====
        console.log("📻 [CHECK 3] Validating radio button selections...");
        const phaseCards = form.querySelectorAll('.phase-card');
        console.log(`📌 Total phases found: ${phaseCards.length}`);
        
        phaseCards.forEach((card, phaseIndex) => {
            const questions = card.querySelectorAll('.question-block');
            console.log(`  Phase ${phaseIndex + 1}: ${questions.length} questions found`);
            ...
            questions.forEach((qBlock, qIndex) => {
                ...
                if (!checked) {
                    console.warn(`❌ [NO SELECTION] Phase ${phaseIndex + 1}, Question ${qIndex + 1} - No correct answer selected`);
                    const radioMsg = qBlock.querySelector('.radio-validation-msg');
                    if (radioMsg) {
                        radioMsg.classList.remove('hidden');
                    }
                    validationErrors.push(`Phase ${phaseIndex + 1}, Question ${qIndex + 1}: No correct answer selected`);
                    isValid = false;
                } else {
                    console.log(`✅ [SELECTED] Phase ${phaseIndex + 1}, Question ${qIndex + 1} - Answer: ${checkedValue}`);
                }
            });
        });
    ...
    function buildSubmissionParams(form) {
        ...
        // Add all non-file fields (skip unchecked radios)
    ...
    function getSelectedPhaseFiles(form) {
        ...
        const phaseCards = form.querySelectorAll('.phase-card');
        phaseCards.forEach((card, phaseIndex) => {
            const submittedPhaseNum = phaseIndex + 1;
    ...
    async function uploadPhaseFiles(pathId, files) {
        ...
        for (const item of files) {
            ...
            data.append('phaseNumber', item.phaseNumber);
            data.append('attachment', item.file);

            try {
                const uploadResp = await fetch("<%=request.getContextPath()%>/user/create-path/upload-phase-file", {
                    method: 'POST',
                    body: data
                });
                ...
                if (!uploadResp.ok) {
                    uploadFailures.push({
                        ...
                        file: item.file.name,
                        error: uploadText || `HTTP ${uploadResp.status}`
                    });
                    console.error('[UPLOAD][CREATE] Error uploading phase', item.phaseNumber, ':', uploadText);
                } else {
                    uploadSuccessCount++;
                    const parts = uploadText.split('|');
                    if (parts[0] === 'RESOURCE_SAVED' && parts.length >= 3) {
                        console.log('[UPLOAD][CREATE] phase', item.phaseNumber, 'SUCCESS - saved path:', parts[1]);
                    }
                }
            } catch (uploadErr) {
                console.error('[UPLOAD][CREATE] Exception uploading phase', item.phaseNumber, ':', uploadErr.message);
                uploadFailures.push({
                    phase: item.phaseNumber,
                    file: item.file.name,
        ...
        // Report results
        console.log(`[UPLOAD][CREATE] Summary: ${uploadSuccessCount}/${files.length} files uploaded successfully`);
        
        if (uploadFailures.length > 0) {
            let errorMsg = `❌ File Upload Errors (${uploadFailures.length}/${files.length} failed)\n\n`;
            uploadFailures.forEach(f => {
                errorMsg += `Phase ${f.phase}: ${f.file}\n${f.error}\n\n`;
            });
            console.error('[UPLOAD][CREATE] Failures:', errorMsg);
            alert(errorMsg);
            throw new Error(`${uploadFailures.length} file(s) failed to upload`);
        } else if (files.length > 0) {
            console.log('[UPLOAD][CREATE] ✅ All files uploaded successfully');
        }
    }
    ...
    function validatePhaseFiles() {
        ...
        phaseCards.forEach((card, index) => {
            ...
            if (!(hasSelectedFile || hasVisiblePreview)) {
                isValid = false;
                if (uploadWrapper) {
    ...
    /**
     * ✅ CORE LOGIC: Form Submission with Detailed Logging
     */
    function handleUpdateAction() {
        console.log("🚀 [SUBMIT] Form submission initiated...");
        console.log("📊 Current form state:", {
            roadmapTitle: document.querySelector('input[name="roadmapTitle"]').value,
            category: document.querySelector('input[name="category"]').value,
            roadmapDesc: document.querySelector('textarea[name="roadmapDesc"]').value,
            status: document.getElementById('pathStatus').value,
            phases: document.querySelectorAll('.phase-card').length
        });

        if (!validateForm()) {
            console.error("❌ [SUBMIT BLOCKED] Validation failed - showing user alert");
            alert('⚠️ Validation Error:\n\nPlease ensure:\n✓ All required fields are filled\n✓ Correct answer is selected for each question\n\nCheck browser console (F12) for detailed errors');
            return;
        }
        ...
        .then(async response => {
            ...
            if(response.ok || response.status === 200) {
                let pathId = null;
                if (responseText.startsWith('PATH_CREATED:')) {
                    pathId = parseInt(responseText.split(':')[1], 10);
                }

                if (pathId && selectedFiles.length > 0) {
                    await uploadPhaseFiles(pathId, selectedFiles);
                }

                console.log("✅ [SUCCESS] Path created successfully!");
                isChanged = false;
                const successMsg = isPublished ? "Roadmap Published Successfully! 🚀" : "Draft Saved Successfully! ✓";
                toastMsg.innerText = successMsg;
                toast.classList.remove('hidden');
                toast.classList.add('flex', 'animate-pop');
                
                setTimeout(() => {
                    console.log("🔄 Redirecting to career management...");
                ...
                const serverMsg = responseText || `Server returned status ${response.status}`;
        ...
        .catch(err => {
            ...
            // Show detailed error in alert box
            let errorMsg = "❌ Submission Failed\n\n";
            errorMsg += "Error: " + (err.message || "Unknown error occurred");
            errorMsg += "\n\n📋 Check browser console (F12) for more details.";
            errorMsg += "\n\n⚠️ The server may have detailed logs about this error.";
    ...
    window.onbeforeunload = function() {
        if (isChanged) return "You have unsaved changes. Do you want to leave?";
    };
