            ...
            <c:choose>
                <c:when test="${isEnrolled}">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="<%=request.getContextPath()%>/student/progress" class="bg-green-600 hover:bg-green-700 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-2xl font-bold shadow-xl shadow-green-600/20 transition-all flex items-center justify-center gap-2 active:scale-95 whitespace-nowrap inline-flex">
                                <span class="material-icons-round text-lg sm:text-base">play_circle</span>
                                <span class="hidden sm:inline">Continue</span>
                                <span class="sm:hidden">Continue</span>
                            </a>
                    ...
                    <div class="relative pl-0 md:pl-16">
                        ...
                        <div class="phase-card">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
                                <div>
                                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                        Phase ${phase.phaseNumber} • <c:choose><c:when test="${isEnrolled}">Active</c:when><c:otherwise>Locked</c:otherwise></c:choose>
                                    </span>
                                ...
                                <c:if test="${isEnrolled && phaseProgressData != null}">
                                    <div class="text-right">
                                        <div class="text-sm font-bold text-primary">${phaseProgressData.bestScore != null ? phaseProgressData.bestScore : '--'}/100</div>
                            ...
                            <div class="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
                                <c:choose>
                                    <c:when test="${isEnrolled}">
                                        <a href="<%=request.getContextPath()%>/student/module?phase=${phase.phaseId}&path=${careerPath.pathId}" 
...
<script>
    function viewPhaseDetails(phaseId, pathId) {
        ...
        window.location.href = "<%=request.getContextPath()%>/student/module?phase=" + phaseId + "&path=" + pathId;
    }
