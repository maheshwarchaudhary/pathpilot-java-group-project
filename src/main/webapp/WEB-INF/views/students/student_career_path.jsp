            ...
            <div class="bg-white rounded-[2rem] overflow-hidden border border-gray-100 shadow-sm path-card transition-all duration-300 flex flex-col group">
                ...
                <div class="p-8 flex-grow flex flex-col">
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-gray-50 mt-auto">
                        ...
                        <c:choose>
                            <c:when test="${pathInfo.isCreator}">
                                <a href="<%=request.getContextPath()%>/student/view-path?id=${path.pathId}" 
                                   class="text-primary font-bold text-sm hover:underline">
                                    View Path
                                </a>
                            </c:when>
                            <c:when test="${isEnrolled && pathInfo.canCertify}">
                                <a href="<%=request.getContextPath()%>/student/certificate?id=${path.pathId}" 
                                   class="text-green-600 font-bold text-sm hover:underline flex items-center gap-1">
                                    <span class="material-icons-round text-sm">card_membership</span>
                                    Get Certificate
                                </a>
                            </c:when>
                            <c:otherwise>
                                <a href="<%=request.getContextPath()%>/student/view-path?id=${path.pathId}" 
                                   class="text-primary font-bold text-sm hover:underline">${isEnrolled ? 'Resume' : 'Quick View'}</a>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
