...
<head>
    <meta charset="UTF-8">
    <title><c:choose><c:when test="${viewMode == 'quiz'}">${title} Assessment</c:when><c:otherwise>Select Assessment - PathPilot</c:otherwise></c:choose></title>
    ...
    <c:if test="${viewMode == 'select'}">
        <nav class="bg-white border-b border-gray-100 py-5 px-10 flex justify-between items-center shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">${careerPath.title}</p>
                </div>
            </div>
            
            <a href="<%=request.getContextPath()%>/student/course-details/${pathId}" class="text-gray-400 hover:text-red-500 transition-colors">
        ...
        <main class="flex-grow flex items-center justify-center px-6 py-12">
            <div class="max-w-4xl w-full">
                ...
                <div class="mb-12">
                    <span class="inline-block px-4 py-1.5 rounded-full bg-indigo-50 text-[#4913ec] text-[10px] font-black uppercase tracking-widest mb-4">Select Phase</span>
                ...
                <c:choose>
                            ...
                            <c:forEach var="phase" items="${phases}" varStatus="phaseLoop">
                                <div onclick="startQuiz(${phase.phaseId})" class="phase-card">
                                    <div class="flex items-start justify-between mb-4">
                                        <div class="w-12 h-12 rounded-xl bg-[#4913ec] flex items-center justify-center text-white font-bold text-lg">
                                            ${phase.phaseNumber}
                                        </div>
                                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Phase ${phase.phaseNumber}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                        <span class="text-[10px] font-bold text-green-600 uppercase">5 Questions</span>
                    ...
                    <c:otherwise>
                        <div class="text-center py-20">
                            ...
                            <a href="<%=request.getContextPath()%>/student/course-details/${pathId}" class="bg-[#4913ec] hover:bg-[#3a0fb5] text-white px-8 py-3 rounded-2xl font-bold transition-all inline-block">
                                Back to Course
                            </a>
                        </div>
                    </c:otherwise>
        ...
        <nav class="bg-white border-b border-gray-100 py-5 px-10 flex justify-between items-center shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Phase ${phaseId}</p>
                </div>
            </div>
            ...
            <div id="quizBox" class="max-w-2xl w-full">
                ...
                <c:if test="${empty questions}">
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500 mb-6">This phase doesn't have any assessment questions yet.</p>
                        <button onclick="history.back()" class="bg-[#4913ec] hover:bg-[#3a0fb5] text-white px-8 py-3 rounded-2xl font-bold transition-all">
                            Go Back
                        </button>
                    </div>
                </c:if>
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                if (pass) {
                                    ...
                                    window.location.href = "${pageContext.request.contextPath}/student/progress?title=" + encodeURIComponent(title) + "&status=complete";
                                } else {
